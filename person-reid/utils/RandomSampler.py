...
class RandomSampler(sampler.Sampler):
    def __init__(self, data_source, batch_id, batch_image):
        ...
        # find the images corresponding to the same person
        self._id2index = collections.defaultdict(list)
        for idx, path in enumerate(data_source.imgs):
            _id = data_source.get_id(path) # get person id
            self._id2index[_id].append(idx) # add corresponding image id

    def __iter__(self):
        unique_ids = self.data_source.unique_ids # person id
        random.shuffle(unique_ids) # randomize

        imgs = []
        # promise the same number of images for each person id
